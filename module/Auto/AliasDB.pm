# -*- cperl -*-
# -----------------------------------------------------------------------------
# $Id: AliasDB.pm,v 1.10 2003/03/02 02:56:11 topia Exp $
# -----------------------------------------------------------------------------
...
sub setfile {
  # クラスメソッド。
  my ($fpath,$charset) = @_;
  $_shared = Auto::AliasDB->_new($fpath,$charset);
}
...
sub find_alias_with_stdreplace {
  ...
  return add_stdreplace(dup_struct($this->find_alias_prefix($prefix, $flag)), $nick, $name, $host, $prefix);
}
...
sub stdreplace_add {
  ...
  my ($add_alias) = add_stdreplace(undef,
